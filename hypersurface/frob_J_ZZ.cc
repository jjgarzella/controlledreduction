// Copyright 2013-2017 Edgar Costa
// See LICENSE file for license details.

#include "hypersurface.h"


/*
 * returns the coordinates of the p-adic approximation of frob(e_i)
 * using N terms, using reduce_vector_J_poly_ZZ
 */
Vec<ZZ_p> hypersurface::frob_J_ZZ( const int64_t coordinate, const int64_t N,const int64_t loop)
{
    assert(N > 0);
    int64_t e, i, j, m, sum, end, val;
    ZZ_p fact;
    //ZZ factZZ;
    ZZ tmp, remainder;
    ZZ Djm, bin;
    Vec<int64_t> ei, monomial, ones, src, dest, v, y;
    Vec<ZZ> G, Gswap;
    Vec<ZZ_p> F, G_ZZ_p, G0, H_last, *H_temp;
    Mat<ZZ> Mred;
    map< Vec<int64_t>, int64_t, vi64less > *dict_V, *dict_G0;
    map< Vec<int64_t>, Vec<ZZ_p>, vi64less > H, Hnew;
    map< Vec<int64_t>, Vec<ZZ_p>, vi64less >::const_iterator Hit;
    map< Vec<int64_t>, Vec<ZZ_p>, vi64less >::iterator Hnewit;
    map< Vec<int64_t>, ZZ_p, vi64less>::const_iterator it;


//    cout << "This is f: " << dR->f << endl;

    set(fact);

    v.SetLength(n+1);

    ones.SetLength(n+1);
    for( i = 0; i <= n; i++)
        ones[i] = 1;

    ei = dR->coKernels_J_basis[coordinate];
    cout << "calulating frobenius of " << ei << " at coordinate " << coordinate << endl;
    

    compute_fpow(N-1);

    //cout << "This is Fpow: " << fpow << endl;

    if( loop == 0)
    {
        Vec<long> fpow_length;
        fpow_length.SetLength(N);
        for( i = 0; i < N ; i++ )
        {
            fpow_length[i] = fpow.get(i).size();
        }
    }
    sum = 0;
    for(i = 0; i <= n; i++)
        sum += ei[i];

    m = (sum + n + 1)/d;

    dict_V = &dR->tuple_dict[ d * n - n ];
    // H = map of differentials stored in V_{ tau( p * u ) }
    // Stored as a u -> coordinates in V_{ tau( p * u ) }

    H_last.SetLength(binomial(d*n-1,n)); //degree = d*n-n+n-(n+1) = d*n - (n+1)

    for( e = m + N - 1; e > n/p; e--)
    {   
        Hnew.clear();

        if(verbose)
            cout<<"e = "<<e<<endl<<"\tadding new terms\n";

        // add new terms with pole order p*e = p*(m+j)

        if( e >= m )
        {
            j = e - m;
            Djm = 0;
            // Djm = sum( [ binomial(m + i -1, m -1) * binomial(i , j) * (-1) ** j for i in range(j,N) ] )
            for( i = j ; i < N ; i++)
            {
                bin = binomial_ZZ(m + i - 1, m - 1) * binomial(i,j);
                if(j%2 == 0)
                    Djm += bin;
                else
                    Djm -= bin;
            }
            for( it = fpow.get(j).begin(); it != fpow.get(j).end(); ++it)
            {
                monomial = it->first + ei + ones;
                H_temp = &H[monomial];
                H_temp->SetLength( dict_V->size() );

                //cout << "FROB COMPONENTS: " << endl;
                //cout << "---Monomial: " << monomial << endl;
                //cout << "---Djm: " << Djm << endl;
                //cout << "---Recursive: " << it->second << endl;
                //cout << "---Factorial: " << fact << endl;

                (*H_temp)[ (*dict_V)[ p * monomial - tweak( p * monomial, d * n - n) ] ] += conv<ZZ_p>(Djm * rep(it->second) * rep(fact));
                /*
                if( H.find(monomial) == H.end() )
                {
                    H_temp = &H[monomial];
                    H_temp->SetLength( dict_V->size() );
                    for(i = 0; i < dict_V->size(); i++)
                        clear( (*H_temp)[i] );
                }
                H[monomial][ (*dict_V)[ p * monomial - tweak( p * monomial, d * n - n) ] ] += Djm * it->second * fact;
                */
            }
        }

	//if (coordinate == 1)
        //    cout << "This is H: " << H << endl;
        //cout << "---Factorial: " << fact << endl;

        /*
         * before
         * fact = (p * (m + N - 1) - 1)!/(p * e - 1)!
         * after
         * fact = (p * (m + N - 1) - 1)! / max(p * (e-1) - 1,0)!
         */
        //assert( fact == factorial<ZZ_p>( p * (m + N - 1) - 1, p*e ) );
        end = (e > 1) ? p : p - 1;

        for( i = 0; i < end ; i++)
            fact *= p*e - i - 1;
        /*
        if (e > 1)
            assert( fact == factorial<ZZ_p>(p * (m + N - 1) - 1, p*(e-1) ) );
        else
            assert( fact == factorial<ZZ_p>(p * (m + N - 1) - 1) );
        */
        // reduce all terms from pole order d * p * e   to  d * p * (e-1)
        for( Hit = H.begin(); Hit != H.end(); Hit++)
        {
            if( verbose )
                //cout<<"\treducing "<< Hit->first <<endl;
	    //cout << "This is H: " << H << endl;

            G = conv< Vec<ZZ> >(Hit->second);
            // choosing the main reduction direction
            sum = 0;

            for( i = 0; i <= n; i++)
            {
                if(Hit->first[i] > 0)
                {
                    v[i] = 1;
                    sum++;
                }
                else
                    v[i] = 0;
            }

            while( sum < d )
            {
                for(i = 0; i <= n; i++)
                {
                    if(Hit->first[i] > v[i])
                    {
                        v[i]++;
                        sum++;
                        break;
                    }
                }
            }

            //cout << "Starting new reduction chunk" << endl;
	    //cout << "reducing the H " << Hit->first << endl;
	    //cout << "Chose direction v = " << v << endl;
	    

            end = (e > n/p + 1) ? p : p - n%p;
            
	    
            switch(loop) // Always 0
            {
                case 0:
                    i = 0;
		    //cout << "Before reduction chunk: " << G << endl;
                    if( end > d * n - n )
                    {
                        // tweak( p
			//cout << "about to reduce_poly" << endl;
                        dest = p * (Hit->first - v) + (p - end) * v + tweak( (d*n - n) * v, (d*n - n) );
                        assert( dest ==  tweak(p * (Hit->first - v) + (p - end)*v + (d*n - n) * v , d*n - n ) );
                        // dest == tweak( p * Hit->first - (end - (d*n - n))*v, d*n -n)
			//cout << "reducing in direction " << v << "with" << end - (d*n - n) << "iterations" << endl;
                        dR->reduce_vector_J_poly_ZZ(G, dest, v, end - (d * n - n), G);
                        i = end - (d*n - n);
                    }
                    for( ; i < end; i++)
                    {
                        src = tweak(p * Hit->first - i * v, d * n - n);
                        dest = tweak(p * Hit->first - (i + 1) * v, d * n - n);
                        y = src - dest;
			//cout << "reducing in direction " << y << endl;
                        dR->reduce_vector_J_ZZ(Gswap, dest,  y, G);
                        for( j = 0; j < (int64_t) G.length(); j++)
                            rem(G[j], Gswap[j], ZZ_p::modulus() );
//cout << "After Step " << i+1 << ": " << G << endl;
                    }
                    break;


                case 1:
                    for(i = 0; i < end; i++)
                    {
                        src = tweak(p * Hit->first - i * v, d * n - n);
                        dest = tweak(p * Hit->first - (i + 1) * v, d * n - n);
                        y = src - dest;
                        dR->reduce_vector_J_ZZ(Gswap, dest,  y, G);
                        for( j = 0; j < (int64_t) G.length(); j++)
                            rem(G[j], Gswap[j], ZZ_p::modulus() );
                    }
                    break;
                case 2:
                    for(i = 0; i < end; i++)
                    {
                        src = tweak(p * Hit->first - i * v, d * n - n);
                        dest = tweak(p * Hit->first - (i + 1) * v, d * n - n);
                        y = src - dest;
                        Mred = dR->get_reduction_matrix_J_ZZ(dest, y);
                        mul(G, Mred, G);
                        for( j = 0; j < (int64_t) G.length(); j++)
                            rem(G[j], G[j], ZZ_p::modulus() );
                    }
                    break;
            }

                               
            G_ZZ_p = conv< Vec<ZZ_p> >(G);
            if(! IsZero(G_ZZ_p) )
            {
                if(e > n/p + 1)
                {
                    y = Hit->first - v;
                    Hnewit =  Hnew.find(y);
                    if( Hnewit == Hnew.end() )
                        Hnew[y] = G_ZZ_p;
                    else
                        Hnewit->second += G_ZZ_p;
                }
                else
                {
                    sum = 0;
                    for( i = 0; i <= n; i++)
                        sum += dest[i];
    
                    mul(G_ZZ_p, *(dR->get_inclusion_matrix_J(dest)), G_ZZ_p);

                    //cout << "Before the inclusion matrix: " << G << endl;
                    //cout << "After inclusion matrix: " << G_ZZ_p << endl;

                    H_last += G_ZZ_p;
                }
            }
        }
        //reduced all terms from p * e to p * (e - 1)
        
        H.swap(Hnew);
    }

    cout << "(combined) Terms after controlled reduction: " << H_last << endl;

    //if (coordinate == 2) assert(false);

    // H_last has the poles with order n
    // final reduction step for H_last
    mul(F, *(dR->get_final_reduction_matrix_J(n)), H_last);
    for(e = n/p; e != 0 ; e-- )
    {
        assert( fact == factorial<ZZ_p>( p * (m + N - 1) - 1, p*e ) );
        if(e >= m)
        {
            if(verbose)
                cout<<"e = "<<e<<endl<<"\tadding new terms\n";

            dict_G0 = &dR->tuple_dict[ d * p * e - (n + 1) ];
            clear(G0);
            G0.SetLength( dict_G0->size() );

            j = e - m;
            // Djm = sum( [ binomial(m + i -1, m -1) * binomial(i , j) * (-1) ** j for i in range(j,N) ] )
            Djm = 0;
            for( i = j ; i < N ; i++)
            {
                bin = binomial(m + i - 1, m - 1) * binomial(i, j);
                if(j%2 == 0)
                    Djm += bin;
                else
                    Djm -= bin;
            }

            for( it = fpow.get(j).begin(); it != fpow.get(j).end(); it++)
            {
                monomial = p * ( it->first + ei + ones ) - ones;
		cout << "Monomial: " << monomial << endl;
                G0[ (*dict_G0)[ monomial ] ] +=  conv<ZZ_p>(Djm * rep(it->second) * rep(fact));
            }
            mul(G0, (*(dR->get_final_reduction_matrix_J(p * e))), G0);
	    cout << "T matrix: " << (*(dR->get_final_reduction_matrix_J(p * e))) << endl;
            F += G0;
        }
        /*
         * before
         * fact = (p * (m + N - 1))!/(p * e)!
         * after
         * fact = (p * (m + N - 1) - 1)! / max(p * (e-1) - 1,0)!
         */
        
        end = (e > 1) ? p : p - 1;

        for( i = 0; i < end ; i++)
            fact *= p*e - i - 1;
    }

    cout << "e: " << e << endl;
    cout << "F before scaling: " << F << endl;
    assert( fact == factorial<ZZ_p>( p*(m + N - 1) - 1 ) );
    cout << "fact before taking valuation: " << fact << endl;
    val = valuation_of_factorial(p*(m + N - 1) - 1, p);
    cout << "val: " << val << endl;
    DivRem(tmp, remainder,  factorial<ZZ>( p*(m + N - 1) - 1 ) , power_ZZ(p,val));
    assert(IsZero(remainder));
    cout << "Factorial: " << fact << endl;
    cout << "Factorial divided by powers of p: " << tmp << endl;
    inv(fact, to_ZZ_p(tmp) ); // procedural modular inverse, i.e. stores tmp^-1 to fact
    cout << "Factorial: " << fact << endl;
    F *= fact;

    cout << "F after factorial: " << F << endl;
    
    if( n - 1 >= val) {
        F *= to_ZZ_p(power_ZZ(p, n - 1 - val));
    	cout << "F after scaling by power of p: " << F << endl;
    }
    else
    {
        ZZ ppower = power_ZZ(p,val - ( n - 1) );
        for(i = 0; i < (int64_t) F.length(); i++)
        {
            //Not sure if DivRem(F[i], remainder, rep(F[i]), ppower) is safe
            DivRem(tmp, remainder, rep(F[i]), ppower);
            assert( IsZero(remainder) );
            F[i] = to_ZZ_p(tmp);
        }
	cout << "F after dividing by p: " << F << endl;
    }

    cout << "Final result of reduction: " << F << endl;

    return F;
}


